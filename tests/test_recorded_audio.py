...
def load_recording(name: str) -> bytes:
    wav_path = RECORDINGS_DIR / f"{name}.wav"
    if not wav_path.exists():
        pytest.skip(f"Recording not found: {wav_path} (run record_test_audio.py first)")
...
class TestSilenceRecordings:
    ...
    def test_silence_amplitude_is_low(self):
        pcm = load_recording("silence_3s")
        result = run_vad_on_pcm(pcm)
...
class TestWakeWordRecordings:
    def test_jarvis_weather_detected(self):
        pcm = load_recording("wake_jarvis_weather")
        result = run_vad_on_pcm(pcm)
    ...
    def test_robson_command_detected(self):
        pcm = load_recording("wake_robson_command")
        result = run_vad_on_pcm(pcm)
        assert result["speech_starts"] >= 1, f"Expected speech detection, got {result['speech_starts']} starts"
        assert result["max_prob"] >= 0.5, f"Expected high VAD prob, got {result['max_prob']:.4f}"
...
class TestEdgeCaseRecordings:
    def test_thinking_pause_has_speech(self):
        pcm = load_recording("thinking_pause")
        result = run_vad_on_pcm(pcm)
        assert result["speech_starts"] >= 1, f"Expected speech, got {result['speech_starts']} starts"

    def test_whisper_detected(self):
        pcm = load_recording("whisper")
        result = run_vad_on_pcm(pcm)
        assert result["max_prob"] > 0.1, f"Whisper not detected at all, max_prob={result['max_prob']:.4f}"

    def test_loud_speech_detected(self):
        pcm = load_recording("loud_speech")
        result = run_vad_on_pcm(pcm)
    ...
    def test_background_music_wake_word(self):
        pcm = load_recording("background_music")
        result = run_vad_on_pcm(pcm)
        assert result["speech_starts"] >= 1, f"Speech with music not detected"

    def test_continuous_speech_detected(self):
        pcm = load_recording("continuous_ramble")
        result = run_vad_on_pcm(pcm)
